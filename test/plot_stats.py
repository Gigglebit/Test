...
#    print start
    ...
    return ret
def plot_st(fname,SPEED=5,n=1):
#file name, bottleneck link speed, number of queues
...
#SPEED = 5 #50Mbps
    ...
    for i in xrange(len(time)):
        ...
        open(name, 'a').write("%.3f"%reftime[i] + ',' + 
                              "%.3f"%mrate[i]+',' + 
                              "%.3f"%mdelay[i] + ',' + 
                              "%.3f"%loss[i] + ','+
                              "%.0f"%utilink[i]+'\n')
...
#ax.plot(reftime,mrate)
    ...
    plt.title("Link Utilisation on this Q vs Time")
    plt.ylabel("Percentage")
    plt.xlabel("Seconds")
    plt.ylim((0,100))
    plt.grid(True)
